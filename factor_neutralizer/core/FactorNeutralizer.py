...
class FactorNeutralizer:
    ...
    def _fast_save_factor(self, factor_data: pd.DataFrame, output_path: str):
        """快速保存因子数据"""
        try:
            # 使用joblib进行快速序列化
            joblib.dump(factor_data, output_path, protocol=4, compress=('zlib', 3))
        except Exception as e:
            # 回退到标准pickle
    ...
    def _load_price_data(self):
        ...
        if os.path.exists(pkl_file):
            try:
                ...
                if isinstance(price_data, pd.DataFrame):
                    ...
                    print(f"价格数据(PKL)加载完成，形状: {price_data.shape}")
                    print(f"时间范围: {price_data.index[0]} 到 {price_data.index[-1]}")
                    
                    # 记录加载后内存
                    loaded_memory = self._get_memory_usage()
                    print(f"  加载后内存: {loaded_memory['rss_mb']:.2f} MB (+{loaded_memory['rss_mb']-initial_memory['rss_mb']:.2f} MB)")
        ...
        if csv_files:
            try:
                ...
                print(f"价格数据(CSV)加载完成，形状: {price_data.shape}")
                return price_data
            except Exception as e:
                print(f"CSV文件加载失败: {e}")
        ...
        if self.price_dir.endswith('.pkl') and os.path.exists(self.price_dir):
            try:
                ...
                if isinstance(price_data, pd.DataFrame):
                    ...
                    print(f"价格数据(直接PKL)加载完成，形状: {price_data.shape}")
                    return price_data
            except Exception as e:
                print(f"直接PKL文件加载失败: {e}")
    ...
    def _load_factor_data(self):
        ...
        with ThreadPoolExecutor(max_workers=min(8, len(factor_files))) as executor:
            ...
            for future in futures:
                ...
                if error:
                    print(f"  加载因子 '{factor_name}' 失败: {error}")
                    continue
                ...
                if not factor_data.empty:
                    ...
                    print(f"  因子 '{factor_name}' 加载完成，形状: {factor_data.shape}")
                else:
                    print(f"  警告: 因子 '{factor_name}' 数据为空或与价格数据无交集")
        
        print(f"共加载 {len(factors)} 个因子")
        return factors
    ...
    def _load_industry_data(self):
        ...
        try:
            ...
            print(f"行业数据加载完成，股票数量: {len(industry_data)}")
    ...
    def _load_index_data(self):
        ...
        try:
            ...
            if isinstance(index_data, pd.DataFrame):
                ...
                print(f"指数 '{self.index_code}' 数据加载完成，记录数: {len(index_data)}")
                return index_data
            else:
                raise Exception("指数数据格式不正确")
        except Exception as e:
    ...
    def _load_market_value_data(self):
        ...
        try:
            ...
            if isinstance(market_value_data, pd.DataFrame):
                ...
                print(f"市值数据加载完成，形状: {market_value_data.shape}")
                return market_value_data
            else:
                raise Exception("市值数据格式不正确")
        except Exception as e:
            print(f"加载市值数据失败: {e}")
            return None
    ...
    def _vectorized_industry_regression(self, factor_data: pd.DataFrame, industry_series: pd.Series) -> pd.DataFrame:
        ...
        for date in factor_data.index:
            ...
            try:
                ...
                print(f"日期 {date} 向量化回归失败: {e}")
                continue
        
        # 清理大型临时变量
        del industry_dummies
        
        print(f"向量化行业回归完成，处理日期数: {len(valid_dates)}")
    ...
    def _batch_save_factors(self, factors_dict: Dict[str, pd.DataFrame], base_dir: str):
        """批量保存因子数据（I/O优化 - 快速序列化）"""
        print(f"批量保存 {len(factors_dict)} 个因子到 {base_dir}")
        ...
        for output_path, factor_data in save_tasks:
            try:
                ...
                try:
                    ...
                    print(f"保存因子 {factor_name} 最终失败: {e2}")
        
        print(f"批量保存完成，成功保存 {saved_count}/{len(save_tasks)} 个因子")
        return saved_count
    
    def _batch_load_factors(self, factor_dir: str) -> Dict[str, pd.DataFrame]:
        """批量加载因子数据（I/O优化 - 快速序列化）"""
        print(f"批量加载因子数据从 {factor_dir}")
        ...
        for factor_file in factor_files:
            ...
            try:
                ...
                try:
                    ...
                    print(f"加载因子 {factor_name} 最终失败: {e2}")
        
        print(f"批量加载完成，成功加载 {loaded_count}/{len(factor_files)} 个因子")
        return factors
    ...
    def process_all_factors(self, 
        ...
        """处理所有因子（I/O优化版）"""
        print("="*60)
        print(f"开始批量处理所有因子，中性化类型: {neutralization_type}")
        print(f"强制重新处理: {force_reprocess}")
        print("="*60)
        
        # 记录处理开始时的内存
        start_memory = self._get_memory_usage()
        print(f"处理开始内存: {start_memory['rss_mb']:.2f} MB")
        ...
        for factor_name, factor_data in self.factors.items():
            ...
            try:
                ...
                if not should_reprocess and os.path.exists(output_path):
                    print(f"因子 {factor_name} 已存在处理结果，跳过处理")
                    # 加载已存在的结果到内存
                    try:
                        with open(output_path, 'rb') as f:
                            existing_data = pickle.load(f)
                        self.neutralized_factors[factor_name] = existing_data
                        print(f"已加载因子 {factor_name} 的现有结果")
                    except Exception as e:
                        print(f"加载现有结果失败: {e}，将重新处理")
                ...
                if should_reprocess or not os.path.exists(output_path):
                    if neutralization_type == 'industry':
                        ...
                        print(f"中性化类型 {neutralization_type} 暂未实现")
                        continue
        ...
        print(f"\n处理结束内存: {end_memory['rss_mb']:.2f} MB (+{end_memory['rss_mb']-start_memory['rss_mb']:.2f} MB)")
    ...
    def _industry_rotation_analysis(self):
        """行业轮动分析"""
        print("进行行业轮动分析...")
            ...
            try:
                ...
                for quarter_end in quarterly_data.index:
                    try:
                        ...
                        print(f"处理季度 {quarter_end} 时出错: {e}")
                        continue
                
                industry_rotation_results[factor_name] = industry_exposure_by_quarter
                
            except Exception as e:
                print(f"处理因子 {factor_name} 的行业轮动分析时出错: {e}")
                continue
        ...
        try:
            ...
            print(f"行业轮动分析结果保存到: {output_path}")
        except Exception as e:
            print(f"保存行业轮动结果时出错: {e}")
    ...
    def _batch_save_figures(self, figure_tasks: list):
        ...
        print(f"批量异步保存 {len(figure_tasks)} 个图片")
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    if success:
                        ...
                        print(f"图片保存失败: {error}")
                except Exception as e:
                    print(f"批量保存中出错: {e}")
        
        print("批量异步保存完成")
    
    def _visualize_industry_rotation(self, industry_rotation_results: Dict):
        """可视化行业轮动（优化版 - 减少图形复杂度）"""
        for factor_name, quarterly_data in industry_rotation_results.items():
            ...
            # 填充NaN为0
            rotation_df = rotation_df.fillna(0)
            ...
            self._async_save_figure(fig, output_path, dpi=150)
            
            # 清理内存
            del rotation_df
            self._cleanup_memory()
    
    def _market_value_rotation_analysis(self):
        """市值轮动分析"""
        print("进行市值轮动分析...")
        ...
        print(f"市值轮动分析结果保存到: {output_path}")
    
    def _visualize_market_value_rotation(self, market_value_rotation_results: Dict):
        """可视化市值轮动"""
        ...
        print(f"市值轮动图保存到: {output_path}")
    
    def _visualize_index_rotation(self, index_rotation_results: Dict):
        """可视化指数轮动"""
...
def main():
    ...
    print("配置信息:")
    print(f"  价格数据: {config['price_dir']} (PKL格式)")
    print(f"  行业数据: {config['industry_file']}")
    print(f"  指数数据: {config['index_file']}")
    print(f"  市值数据: {config['market_value_file']}")
